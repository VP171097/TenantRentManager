...
## Verification
### GitHub build lockfile fix
The user reported `actions/setup-node` failing because `yarn.lock` was absent from the pushed checkout. Inspection confirmed it existed locally but was untracked (`git ls-files yarn.lock` empty). The fix includes the existing resolved lockfile in version control, pins the Yarn Classic package-manager version, explicitly points caching at the root lockfile, and adds an early actionable missing-lockfile check. No app code, database configuration, schema or financial values changed. Verify with a clean tracked-file export and the workflow's install/typecheck/lint/test/build commands; actual GitHub Actions rerun is separate from local verification.
Verified after the fix: a clean index export with no node_modules or .env.local installed successfully using Yarn 1.22.22 `--frozen-lockfile`; typecheck, lint (15 non-blocking existing warnings, zero errors), all 50 tests, and the production build all exited 0. Tracked, working-tree, and clean-export lockfile hashes matched. Public preview HTTP 200 remained healthy. Local validation used Node 24.19.0; the workflow retains Node 22. No remote GitHub Actions run was triggered or claimed. The user must push this new change (including yarn.lock); rerunning the old failing commit cannot include the fix.
