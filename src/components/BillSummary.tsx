    ...
    <div className="card">
      ...
      <div className="mt-4 space-y-1 text-sm">
        {rows.map(([label, value]) => (
          <div key={label} className="flex justify-between text-slate-600 dark:text-slate-300">
            <span>{label}</span>
            <span>{formatINR(value)}</span>
          </div>
        ))}
      </div>
      <div className="mt-3 border-t border-slate-200 dark:border-slate-700 pt-3 space-y-1">
        ...
        <div className="flex justify-between font-bold">
          ...
          <span className={bill.balance > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}>{formatINR(Math.abs(bill.balance))}</span>
        </div>
      </div>
    </div>
  )
