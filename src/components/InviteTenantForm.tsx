 ...
 * password directly (CreateTenantLoginForm). The link can be shared
 * through any channel (WhatsApp, SMS, email, in person); this app never
 * sends it automatically. */
export function InviteTenantForm({ tenant }: { tenant: Tenant }) {
  const queryClient = useQueryClient()
  const [copied, setCopied] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const inviteMutation = useMutation({
    mutationFn: () => generateTenantInvite(tenant.id),
    onSuccess: () => {
      setCopied(false)
  ...
  const revokeMutation = useMutation({
    mutationFn: () => revokeTenantInvite(tenant.id),
  ...
  const isExpired = tenant.invite_token_expires_at ? new Date(tenant.invite_token_expires_at).getTime() < Date.now() : true
  ...
  async function handleCopy() {
    ...
    try {
      ...
      // text is still visible on screen for manual copying.
    }
  }
  ...
  return (
    <div className="card max-w-md space-y-3">
      <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
        Or invite the tenant to set up their own login
      </p>
      <p className="text-sm text-slate-500 dark:text-slate-400">
        Share this link with the tenant (WhatsApp, SMS, email — any way you like). They'll set their own password.
      </p>
